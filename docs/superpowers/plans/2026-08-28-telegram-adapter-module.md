...
- Multi-currency support (originally scoped to the group's single `defaultCurrency` for `/balances` and `/list`) — fully resolved 2026-09-02, in two passes:
  - Pass 1: `/expenses` (formerly `/list`), `/settlements`, `/history`, and `/delete`. `ExpenseRepository.listActive`/`SettlementRepository.listActive` no longer take a `currency` filter at all — they return every active row for the group, and each of those four commands already formats/matches per-row using that row's own `currency` field, so expenses and settlements in any currency now show up (and can be deleted) regardless of the group's `defaultCurrency`. `/history` treats each currency as an independent ledger (`computeBalanceHistory`'s all-currencies overload runs the fold once per currency and merges the results chronologically) — a settlement that fully clears one currency's ledger says "Everyone's settled up in EUR", not a bare "Everyone's settled up", since other currencies may still have open balances.
  - Pass 2: `/balance` and `/settle_suggest`. Both now compute `computeBalances`/`simplifyDebts` once per distinct currency the group has ever used, and `formatBalances`/`formatSettleSuggestions` take a `Map<String, List<DebtPayment>>` (one entry per currency) instead of a single currency + payment list — a currency with no relevant payments contributes no lines, so already-settled currencies are silently skipped rather than shown as a separate "settled up" block.
