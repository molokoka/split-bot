...
class ExposedMemberRepository(
    ...
    override suspend fun create(member: Member): Unit =
    ...
    override suspend fun find(id: MemberId): Member? =
        withContext(Dispatchers.IO) {
            suspendTransaction(db) {
                MemberTable
                    .selectAll()
    ...
    override suspend fun findByGroup(groupId: GroupId): List<Member> =
        withContext(Dispatchers.IO) {
            suspendTransaction(db) {
...
internal fun ResultRow.toMember() =
    Member(
