...
class HistoryCommandSpec : StringSpec({
    ...
    "shows each member's running balance after every split and settlement, oldest first" {
        withTestDatabase { db ->
            ...
            val command = HistoryCommand(groupRepository, memberRepository, expenseRepository, settlementRepository, platformDirectory, telegramApi)
    ...
    "explains there's nothing yet" {
        withTestDatabase { db ->
            ...
            telegramApi.sentRichMessages shouldBe listOf(
                -100L to InputRichMessage(blocks = listOf(RichBlockParagraph("No history yet — use /split or /settle to get started."))),
            )
        }
    }
})
